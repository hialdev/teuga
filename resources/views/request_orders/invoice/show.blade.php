        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    @forelse($roInvoice->transactions as $trx)
                    ...
                    @empty
                    <form action="{{route('request-order.invoice.generateTrx')}}" method="POST">
                        @csrf
                        <input type="hidden" name="id" value="{{$roInvoice->id}}">
                        <button class="btn btn-primary mt-2 w-100">Generate Transaksi Permintaan (Piutang ke {{ $roInvoice->requestOrder->client->name }})</button>
                    </form>
                    @endforelse
    ...
    <div class="row">
        <div class="col-12">
            <div class="mb-3 d-flex align-items-center gap-2 justify-content-between">
                <h4 class="mb-0">Pembayaran (Piutang)</h4>
                ...
                <a href="{{ urlApp('OSN', '/invoice/request-order/'.$roInvoice->id) }}" target="_blank" class="btn btn-primary btn-al-primary"
                    >Kelola</a>
            </div>
        </div>
                                    ...
                                    <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                        <div class="modal-content">
                                            <div class="modal-header pb-0 d-flex align-items-center">
                                                <h4 class="modal-title" id="myLargeModalLabel">
                                                    Tentukan Dimana (Akun) Pembayaran diterima
                                                </h4>
                                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                            </div>
                                            <div class="modal-body mt-0">
                                                <form action="{{ route('request-order.payment.generateTrx', ['id' => $roInvoice->id, 'pay_id' => $pay->id]) }}" method="POST">
                                                    @csrf

                                                    <label for="text" class="form-label">Akun Penerimaan</label>
