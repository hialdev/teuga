    ...
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Invoice Permintaan (PO Client)</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('home') }}">Dashboard</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Manage Invoice Permintaan (PO Client)</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
        ...
        <div class="card-body">
                ...
                <div class="row align-items-end mb-3 flex-wrap">
                    ...
                    <div class="col-md-3 mb-2">
                        ...
                        <select name="field" id="field" class="form-select">
                            @foreach (getModelAttributes('RequestOrderInvoice', []) as $atr)
            ...
            <div class="table-responsive">
                <table class="table border text-nowrap mb-0 align-middle">
                    ...
                    <tbody>
                        @forelse ($roInvoices as $roInvoice)
                            <tr>
                                <td>
                                    <div>
                                        ...
                                        <h6 class="fs-2 fw-semibold text-success mb-1" style="">
                                            {{ \Carbon\Carbon::parse($roInvoice->created_at)->format('d F Y') }}</h6>
                                    </div>
                                    <div>
                                        <div class="fw-normal fs-1 text-muted" style="white-space:normal;">Kode Invoice
                                        </div>
                                        <a class="d-flex align-items-center gap-1 mb-1" href="{{route('request-order.invoice.show', $roInvoice->id)}}">
                                            <h6 class="fw-semibold text-primary mb-0" style="">{{ $roInvoice->code }}</h6>
                                    ...
                                    <div>
                                        ...
                                        <h6 class="fw-semibold fs-2 text-{{ $statusPembayaran[$roInvoice->payment_status]['color'] }} mb-1" style="">{{ $statusPembayaran[$roInvoice->payment_status]['label'] }}</h6>
                                    </div>
                                </td>
                                <td>
                                    @if($roInvoice->requestOrder)
                                        <a href="{{route('request-order.setting', $roInvoice->requestOrder->id)}}" target="_blank" class="border border-primary p-1 px-2 rounded-2 d-inline-flex align-items-center fs-2 mb-1 gap-2">
                                            <i class="ti ti-building-skyscraper mb-0 fs-3"></i> {{ $roInvoice->requestOrder->code }}
                                        </a>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <h6 class="fw-semibold fs-2 text-{{ $status[$roInvoice->requestOrder->status]['color'] }} mb-1" style="">{{ $status[$roInvoice->requestOrder->status]['label'] }}</h6>
                                        </div>
                                        <div>
                                ...
                                <td>
                                    <div class="dropdown dropstart">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            ...
                                            <li>
                                                <button type="button" class="dropdown-item d-flex text-danger align-items-center gap-3"
                                            ...
                                            <li>
                                                <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                                    data-bs-toggle="modal" data-bs-target="#deleteModal-{{$roInvoice->id}}"><i
                                    ...
                                    <!-- Delete Modal -->
                                    <div id="deleteModal-{{$roInvoice->id}}" class="modal fade" tabindex="-1"
                                            ...
                                            <div class="modal-content p-3 modal-filled bg-danger">
                                                <div class="modal-header modal-colored-header text-white">
                                                    <h4 class="modal-title text-white" id="danger-header-modalLabel">
                                                        Yakin ingin menghapus Invoice Permintaan ke Principal ?
                                                    </h4>
                                                ...
                                                <div class="modal-body" style="width: fit-content; white-space:normal">
                                                    <h5 class="mt-0 text-white">Invoice Permintaan ke Principal {{$roInvoice->code}} akan dihapus</h5>
                                                    <p class="text-white">Segala data yang berkaitan dengan Invoice Permintaan ke Principal tersebut juga akan dihapus secara permanen.</p>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">
                                                        Close
                                                    </button>
                                                    <form action="{{route('request-order.invoice.destroy', $roInvoice->id)}}" method="POST">
                                                        @csrf
                                                        @method('delete')
