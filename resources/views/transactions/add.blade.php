    ...
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Input Transaksi Baru</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('transaction.index') }}">Transaksi</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Input Transaksi Baru</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
        ...
        <form action="{{ route('transaction.store') }}" method="POST">
            ...
            <div class="card mb-4">
                <div class="px-4 py-3 border-bottom">
                    <h1 class="card-title fw-semibold mb-0">Input Transaksi Baru</h1>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                            ...
                            <div style="flex-grow:1">
                                <select name="transaction_period_id" id="transaction_period_id" class="select2-period form-select">
                                    ...
                                    @foreach ($periods as $period)
                                        <option value="{{$period->id}}" {{\Carbon\Carbon::parse($period->start_date)->format('Y') == \Carbon\Carbon::parse(request()->get('trxdate'))->format('Y') ? 'selected' : ''}} data-year="{{\Carbon\Carbon::parse($period->start_date)->format('Y')}}" {{$period->id == old('transaction_period_id') ? 'selected' : ''}}>{{$period->name.' - '.\Carbon\Carbon::parse($period->start_date)->format('d M Y').' s.d '.\Carbon\Carbon::parse($period->end_date)->format('d M Y') }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div>
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" id="date" name="date" value="{{ old('date', request()->get('trxdate', date('Y-m-d'))) }}" required>
                        </div>
                        <div class="mb-3">
                            <label for="description_primary" class="form-label">Deskripsi</label>
                            <textarea class="form-control" id="description_primary" name="description_primary" placeholder="Masukkan deskripsi transaksi">{{ old('description_primary', request()->get('trxdesc')) }}</textarea>
                        </div>
                    </div>
                    <hr>
                    ...
                    <div class="table-responsive">
                        <table class="table border text-nowrap mb-0 align-middle" id="transaction-details">
                            ...
                            <tbody>
                                @forelse(old('account_id',[]) as $index => $accountId)
                                <tr class="">
                                    <td>
                                        <div style="width:20em">
                                            <select name="account_id[]" class="form-control select2-normal" required>
                                                ...
                                                @foreach($accounts as $account)
                                                    <option value="{{ $account->id }}" {{$account->id == $accountId ? 'selected' : ''}}>{{ $account->full_code }} - {{ $account->account_name. ',Balance : '.formatRupiah($account->balance)}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </td>
                                ...
                                <tr class="">
                                    <td>
                                        <div style="width:20em">
                                            <select name="account_id[]" class="form-control select2-normal" required>
                                                <option value="">Pilih Akun</option>
                                                @foreach($accounts as $account)
                                                    <option value="{{ $account->id }}" {{ $account->id == request()->get('trxaccount') ? 'selected' : ''}}>{{ $account->full_code }} - {{ $account->account_name .', Balance: '.formatRupiah($account->balance) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </td>
                                    <td>
                                        <div style="width:100%;min-width:13em;max-width:15em">
                                            <input type="text" name="description[]" class="form-control" placeholder="Keterangan Transaksi" value="{{request()->get('trxdesc')}}">
                                        </div>
                                    </td>
                                    <td>
                                        <div style="width:100%;min-width:13em;max-width:15em">
                                            <input type="text" name="debit[]" class="form-control input-rupiah" placeholder="Rp" value="{{formatRupiah(request()->get('trxprice'))}}">
                                        </div>
                                    </td>
                                    <td>
                                        <div style="width:100%;min-width:13em;max-width:15em">
                        ...
                        <td>
                            <div style="width:20em">
                                <select name="account_id[]" class="form-control select2-normal" required>
                                    <option value="">Pilih Akun</option>
                                    @foreach($accounts as $account)
                                        <option value="{{ $account->id }}">{{ $account->full_code }} - {{ $account->account_name. ',Balance : '.formatRupiah($account->balance) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </td>
