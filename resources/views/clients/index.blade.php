...
@section('content')
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Client</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('home') }}">Dashboard</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Semua Client</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
    ...
    <div class="mb-3 d-flex align-items-center gap-2 justify-content-between">
        ...
        <div style="aspect-ratio:1/1; width:3em; height:3em"
            ...
            {{ count($clients) }}</div>
        <a href="{{ route('client.add') }}" class="btn btn-primary btn-al-primary">Tambah</a>
                ...
                <table class="table border text-nowrap mb-0 align-middle">
                    <thead class="text-dark fs-4">
                        <tr>
                            ...
                            <th>
                                <h6 class="fs-3 fw-semibold mb-0">Alamat Proyek / Lainnya</h6>
                            </th>
                            <th>
                    ...
                    <tbody>
                        @forelse ($clients as $client)
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center" style="width:15em">
                                        <img src="{{ $client->image ? asset('/storage/'.$client->image) : '/assets/images/profile/user-1.jpg' }}"
                                            class="rounded-2" alt="Client Image {{ $client->name }}" style="width: 4em" />
                                        <div class="ms-3">
                                ...
                                <td>
                                    <a href="{{route('client.setting', $client->id).'#pic'}}" class="fs-2 text-center d-inline-flex p-2 px-3 align-items-center gap-2 text-primary bg-primary-subtle rounded-3"
                                    >
                                        <i class="fs-4 ti ti-user-circle"></i> {{ count($client->pics) }} PIC / Sales
                                    </a>
                                </td>
                                <td>
                                    <button type="button" class="dropdown-item fs-2 text-center d-inline-flex p-2 px-3 align-items-center gap-2 bg-secondary text-white rounded-3"
                                            data-bs-toggle="modal" data-bs-target="#addressOtherModal-{{$client->id}}"><i
                                                class="fs-4 ti ti-map-2"></i> {{ count($client->addresses) }} Alamat Lainnya</button>
                                    
                                    <!-- Add Address modal -->
                                    <div class="modal fade " id="addressOtherModal-{{$client->id}}" tabindex="-1" aria-labelledby="vertical-center-modal"
                                        aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
                                            <div class="modal-content">
                                                <div class="modal-header d-flex align-items-center">
                                                    <h4 class="modal-title" id="myLargeModalLabel">
                                                        Alamat Proyek / Lainnya
                                                    </h4>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body pt-0">
                                                    @foreach ($client->addresses as $address)
                                                        <div class="d-flex align-items-center gap-2 {{$loop->index+1 != $client->addresses->count() ? 'mb-2' : ''}} p-4 rounded-3 bg-secondary-subtle">
                                ...
                                <td>
                                    <div class="dropdown dropstart">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li>
                                                <a href="{{route('client.setting', $client->id).'#data-client'}}" class="dropdown-item text-primary d-flex align-items-center gap-3"><i
                                                        class="fs-4 ti ti-settings"></i>Kelola</a>
                                            </li>
                                            <li>
                                                <a href="{{route('client.setting', $client->id).'#alamat-proyek'}}" class="dropdown-item text-secondary d-flex align-items-center gap-3"><i
                                                        class="fs-4 ti ti-map-2"></i>Kelola Alamat</a>
                                            </li>
                                            <li>
                                                <a href="{{route('client.setting', $client->id).'#pic'}}" class="dropdown-item text-secondary d-flex align-items-center gap-3"><i
                                                        class="fs-4 ti ti-user-circle"></i>Kelola PIC</a>
                                            </li>
                                            <li>
                                                <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                                    data-bs-toggle="modal" data-bs-target="#deleteModal-{{$client->id}}"><i
                                    ...
                                    <!-- Delete Modal -->
                                    <div id="deleteModal-{{$client->id}}" class="modal fade" tabindex="-1"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
                                            <div class="modal-content p-3 modal-filled bg-danger">
                                                ...
                                                <div class="modal-body" style="width: fit-content; white-space:normal">
                                                    <h5 class="mt-0 text-white">Client {{$client->title}} akan dihapus</h5>
                                                    <p class="text-white">Segala data yang berkaitan dengan Client tersebut juga akan dihapus secara permanen.</p>
                                                </div>
