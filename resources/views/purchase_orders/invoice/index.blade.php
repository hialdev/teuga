    ...
    <div class="card bg-info-subtle shadow-none position-relative overflow-hidden mb-4">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">Invoice Pembelian (PO Principal)</h4>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a class="text-muted text-decoration-none" href="{{ route('home') }}">Dashboard</a>
                            </li>
                            <li class="breadcrumb-item" aria-current="page">Manage Invoice Pembelian (PO Principal)</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-3">
        ...
        <div class="card-body">
                ...
                <div class="row align-items-end mb-3 flex-wrap">
                    ...
                    <div class="col-md-3 mb-2">
                        ...
                        <select name="field" id="field" class="form-select">
                            @foreach (getModelAttributes('PurchaseOrderInvoice', []) as $atr)
            ...
            <div class="table-responsive">
                <table class="table border text-nowrap mb-0 align-middle">
                    ...
                    <tbody>
                        @forelse ($poInvoices as $poInvoice)
                            <tr>
                                <td>
                                    <div>
                                        ...
                                        <h6 class="fs-2 fw-semibold text-success mb-1" style="">
                                            {{ \Carbon\Carbon::parse($poInvoice->created_at)->format('d F Y') }}</h6>
                                    </div>
                                    <div>
                                        <div class="fw-normal fs-1 text-muted" style="white-space:normal;">Kode Invoice
                                        </div>
                                        <a class="d-flex align-items-center gap-1 mb-1" href="{{route('purchase-order.invoice.show', $poInvoice->id)}}">
                                            <h6 class="fw-semibold text-primary mb-0" style="">{{ $poInvoice->code }}</h6>
                                    ...
                                    <div>
                                        ...
                                        <h6 class="fw-semibold fs-2 text-{{ $statusPembayaran[$poInvoice->payment_status]['color'] }} mb-1" style="">{{ $statusPembayaran[$poInvoice->payment_status]['label'] }}</h6>
                                    </div>
                                </td>
                                <td>
                                    @if($poInvoice->purchaseOrder)
                                        <a href="{{route('purchase-order.setting', $poInvoice->purchaseOrder->id)}}" target="_blank" class="border border-primary p-1 px-2 rounded-2 d-inline-flex align-items-center fs-2 mb-1 gap-2">
                                            <i class="ti ti-building-factory mb-0 fs-3"></i> {{ $poInvoice->purchaseOrder->code }}
                                        </a>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <h6 class="fw-semibold fs-2 text-{{ $status[$poInvoice->purchaseOrder->status]['color'] }} mb-1" style="">{{ $status[$poInvoice->purchaseOrder->status]['label'] }}</h6>
                                        </div>
                                        <div>
                                            <div class="fw-normal fs-1 text-muted" style="">Status Penagihan Invoice
                                            </div>
                                            <h6 class="fw-semibold fs-2 text-{{ $statusInvoice[$poInvoice->purchaseOrder->generate_invoice]['color'] }} mb-1" style="">{{ $statusInvoice[$poInvoice->purchaseOrder->generate_invoice]['label'] }}</h6>
                                        </div>
                                    @else
                                ...
                                <td>
                                    <div>
                                        ...
                                        <h6 class="fs-2 fw-semibold text-secondary mb-1" style="">{{ formatRupiah($poInvoice->product_qty_price['total_price_taxed']) }}</h6>
                                    </div>
                                    <div>
                                ...
                                <td>
                                    <div class="dropdown dropstart">
                                        ...
                                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                            <li>
                                                <a href="{{route('purchase-order.invoice.show', $poInvoice->id)}}" class="dropdown-item d-flex align-items-center gap-3 bg-primary text-white"><i
                                                        class="fs-4 ti ti-credit-card"></i>Kelola</a>
                                            </li>
                                            <li>
                                                <button type="button" class="dropdown-item d-flex align-items-center gap-3"
                                                    data-bs-toggle="modal" data-bs-target="#deleteModal-{{$poInvoice->id}}"><i
                                    ...
                                    <!-- Delete Modal -->
                                    <div id="deleteModal-{{$poInvoice->id}}" class="modal fade" tabindex="-1"
                                        ...
                                        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
                                            <div class="modal-content p-3 modal-filled bg-danger">
                                                ...
                                                <div class="modal-body" style="width: fit-content; white-space:normal">
                                                    <h5 class="mt-0 text-white">Invoice Pembelian ke Principal {{$poInvoice->code}} akan dihapus</h5>
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <form action="{{route('purchase-order.invoice.destroy', $poInvoice->id)}}" method="POST">
                                                        @csrf
                                                        @method('delete')
