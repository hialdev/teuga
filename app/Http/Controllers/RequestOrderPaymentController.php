...
class RequestOrderPaymentController extends Controller
{
    ...
    private function syncStatus($invoice){
        try {
            if($invoice->remaining_payment == $invoice->product_qty_price['total_price_taxed']){
                $invoice->payment_status = '0';
            }else if($invoice->remaining_payment < $invoice->product_qty_price['total_price_taxed'] && $invoice->remaining_payment > 0){
